...
void	 parse(int fd, t_parse **cub_file, char *filename)
{
	...
	while ((ret = read(fd, &c, 1)) != 0 && ret != -1)
		mal++;
	close(fd);
	if (!((*cub_file)->p_file = (char *)malloc((mal + 1) * sizeof(char))))
		ft_error("cannot allocate memmory for parsing the given configuration file");
	
	if ((fd = open(filename, O_RDONLY)) < 0)
		ft_error("cannot open the given configuration file");
	if (read(fd, (*cub_file)->p_file, mal) < 0)
		ft_error("cannot read the given configuration file");
	(*cub_file)->p_file[mal] = '\0';
...
int	get_map(t_parse **cub_file, int i) // ok
{
	...
	while (tmp[i])
	{
		...
		while (tmp[i] == '\n')
		{
			i++;
			if (i - tmp_i > 1 && tmp[i] != '\n' && tmp[i] != '\0')
				ft_error("*.cub file has errors. MAP CANNOT have \\n in the map and it shoulb the last argument in the config file *.cub");
			if (i - tmp_i == 0 && tmp[i] != '\n')
				i--;
		}
		i = tmp_i;
