...
void					convert_map_NWSE(t_parse **cub_file)
{
	...
	while ((*cub_file)->map2d[y][x]) // запад и восток
	{
		...
		{
			if (x % SCALE == 0)
				if ((*cub_file)->map2d[y][x] == '1')
					(*cub_file)->map2d[y][x] = '9';
			if ((x - 1) % SCALE != 0 && x > 0)
				if ((*cub_file)->map2d[y][x - 1] == '1')
	...
	x = 0;
	y = SCALE;
	while ((*cub_file)->map2d[y][x]) // север
	{
		while ((*cub_file)->map2d[y - 1][x]) // 6 - север, 7 - юг, 8 - запад, 9 - восток
		{
			if ((*cub_file)->map2d[y - 1][x] == '1' || (*cub_file)->map2d[y - 1][x] == '8' || (*cub_file)->map2d[y - 1][x] == '9')
				(*cub_file)->map2d[y - 1][x] = '6';
			x++;
		}
		x = 0;
		y = y + SCALE * 2;
		if (y >= (*cub_file)->map_y * SCALE)
			break ;
	}
	...
	x = 0;
	y = SCALE * 2;
	while ((*cub_file)->map2d[y][x]) // юг
	{
		while ((*cub_file)->map2d[y][x]) // 6 - север, 7 - юг, 8 - запад, 9 - восток
		{
			if ((*cub_file)->map2d[y][x] == '1' || (*cub_file)->map2d[y][x] == '8' || (*cub_file)->map2d[y][x] == '9')
				(*cub_file)->map2d[y][x] = '7';
			x++;
		}
		x = 0;
		y = (SCALE * 2) * ++i;
